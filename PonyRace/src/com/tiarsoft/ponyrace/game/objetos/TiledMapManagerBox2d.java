...
import com.tiarsoft.ponyrace.game.objetos.BloodStone.Tipo;
...
public class TiledMapManagerBox2d {
	...
	public void crearFisicos(Map map, String layerName) {
		...
		while (objectIt.hasNext()) {
			...
			if (tipo == null)
				...
				float x = (rectangle.x + rectangle.width * 0.5f) * m_units;
				float y = (rectangle.y + rectangle.height * 0.5f) * m_units;
				oWorld.arrBloodStone.add(new BloodStone(x, y, Tipo.chica, oWorld.oRan));
				continue;
			}
				...
				float x = (rectangle.x + rectangle.width * 0.5f) * m_units;
				float y = (rectangle.y + rectangle.height * 0.5f) * m_units;
				oWorld.arrBloodStone.add(new BloodStone(x, y, Tipo.mediana, oWorld.oRan));
				continue;
			}
				...
				float x = (rectangle.x + rectangle.width * 0.5f) * m_units;
				float y = (rectangle.y + rectangle.height * 0.5f) * m_units;
				oWorld.arrBloodStone.add(new BloodStone(x, y, Tipo.grande, oWorld.oRan));
				continue;
			}
			...
			if (tipo.equals("bandera1") || tipo.equals("caer") || tipo.equals("saltoDerecha") || tipo.equals("bandera") || tipo.equals("regresoHoyo") || tipo.equals("caminarIzquierda") || tipo.equals("caminarDerecha") || tipo.equals("saltoIzquierda") || tipo.equals("salto")) {
			...
			if (tipo.equals("bandera1")) {
				if (properties.get("jump").equals("left"))
					body.setUserData(new Bandera1(oWorld, Bandera1.TipoAccion.saltoIzq));
				else if (properties.get("jump").equals("right"))
					body.setUserData(new Bandera1(oWorld, Bandera1.TipoAccion.saltoDer));
	...
	private void crearFogata(MapObject object) {
		...
		Fogata oFogata;
		oFogata = new Fogata(as.getBoundingRectangle().width / 2f * m_units + as.getBoundingRectangle().x * m_units + xLost, as.getBoundingRectangle().height / 2f * m_units + as.getBoundingRectangle().y * m_units + yLost - .15f, oWorld.oRan);
